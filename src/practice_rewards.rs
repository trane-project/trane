...
impl LocalPracticeRewards {
    ...
    fn record_unit_reward_helper(&mut self, reward: &UnitReward) -> Result<bool> {
        ...
        let _ = stmt.execute(params![reward.unit_id.as_str()])?;
...
impl PracticeRewards for LocalPracticeRewards {
    ...
    fn record_unit_reward(&mut self, reward: &UnitReward) -> Result<bool, PracticeRewardsError> {
        self.record_unit_reward_helper(reward)
            .map_err(|e| PracticeRewardsError::RecordReward(reward.unit_id, e))
    }
...
mod test {
    ...
    fn basic() -> Result<()> {
        ...
        practice_rewards.record_unit_reward(&UnitReward {
            unit_id,
            value: 3.0,
            weight: 1.0,
            timestamp: 1,
