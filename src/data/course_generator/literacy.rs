...
impl LiteracyFile {
    ...
    pub fn open_md(path: &Path) -> Result<String> {
        ...
        reader
            ...
            .context(format!("cannot read knowledge base file {display}"))?;
        Ok(contents)
    }
}
...
/// format `<short_id>.lesson`. Each directory must contain at least one example file, which are
/// files with the suffix `.example.md`. The optional exceptions are files with the suffix
/// `.exception.md`.
///
